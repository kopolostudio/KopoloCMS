 ...
 */
define('HTTP_UPLOAD_DEFAULT_CHMOD', 0664);
    ...
    function HTTP_Upload_Error($lang = null, $html = false)
    {
        ...
        $this->error_codes = array(
            'TOO_LARGE' => array(
                ...
                'ru' => "Размер файла слишком велик. Максимально допустимый размер: $maxsize bytes.",
                ),
            'MISSING_DIR' => array(
                ...
                'ru'    => 'Отсутствует каталог назначения.',
                ),
            'IS_NOT_DIR' => array(
                ...
                'ru'    => 'Каталог назначения не существует.',
                ),
            'NO_WRITE_PERMS' => array(
                ...
                'ru'    => 'Невозможно записать файл в каталог назначения',
                ),
            'NO_USER_FILE' => array(
            ...
            'BAD_FORM' => array(
                ...
                'ru' => 'HTML-форма не содержит необходимые теги METHOD="post" ENCTYPE="multipart/form-data"',
                ),
            'E_FAIL_COPY' => array(
                ...
                'ru' => 'Ошибка при копировании временного файла',
                ),
            'E_FAIL_MOVE' => array(
                ...
                'ru'    => 'Невозможно переместить файл.',
                ),
            'FILE_EXISTS' => array(
            ...
            'CANNOT_OVERWRITE' => array(
                ...
                'ru' => 'Выходной Файл уже существует и не может быть перезаписан',
                ),
            'NOT_ALLOWED_EXTENSION' => array(
                ...
                'ru' => 'Не разрешенное расширение файла',
                ),
            'PARTIAL' => array(
                ...
                'pt_BR' => 'O arquivo nгo foi enviado por completo.',
                'ru'    => 'Файл был частично загружен.',
                ),
            'ERROR' => array(
                'es'    => 'Error en subida:',
                'en'    => 'Upload error:',
            ...
            'DEV_NO_DEF_FILE' => array(
                'es'    => 'No estб definido en el formulario este nombre de fichero como &lt;input type="file" name=?&gt;.',
                ...
                'ru'    => 'Имя файла не заданно в поле формы.',
                )
        );
    }
        ...
        foreach ($this->post_files as $userfile => $value) {
            if (is_array($value['name'])) {
                foreach ($value['name'] as $key => $val) {
                    ...
                    $name = trim (basename (' '.$value['name'][$key]));
...
class HTTP_Upload_File extends HTTP_Upload_Error
{
    ...
    {
        $noalpha = 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя';
        $alpha   = 'ABVGDEEJZIIKLMNOPRSTUFHCCWW_I_EUAabvgdeejziiklmnoprstufhccww_i_eua';
