...
//	заменено так как sprintf почему-то не работал с большими данными, тем более если это base64
        $query = "INSERT INTO ".$this->mail_table." (id, create_time, time_to_send, id_user, ip, sender, recipient, headers, body, delete_after_send) VALUES ('".$id."', '".date ('Y-m-d H:i:s')."', '".addslashes ($time_to_send)."', ".addslashes ($id_user).", '".addslashes ($ip)."', '".addslashes ($sender)."', '".addslashes ($recipient)."', '".addslashes ($headers)."', '".addslashes ($body)."', ".$delete_after_send.")";
/*
